...
// ANALYTICS TAB
// ═══════════════════════════════════════════
function AnalyticsTab({ pipeline, searchHistory, appointments, performance }) {
  const stageData = STAGES.map(s => ({ name: s.replace(' ', '\n'), value: pipeline.filter(p => p.stage === s).length })).filter(d => d.value > 0)
  const tierData = [
  ...
  const topCategories = Object.entries(pipeline.reduce((acc, l) => { acc[l.category || 'Unknown'] = (acc[l.category || 'Unknown'] || 0) + 1; return acc }, {})).map(([name, value]) => ({ name, value })).sort((a,b) => b.value - a.value).slice(0, 6)
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div className="card card-p">
          ...
          {stageData.length === 0 ? <p style={{ color: 'var(--text3)', fontSize: 13 }}>No data yet</p> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={stageData} margin={{ top: 0, right: 0, left: -20, bottom: 40 }}>
                ...
                <Tooltip contentStyle={{ background: '#111a0e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#f0f4ed' }} />
                <Bar dataKey="value" fill="#6d8a40" radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        <div className="card card-p">
          ...
          {tierData.length === 0 ? <p style={{ color: 'var(--text3)', fontSize: 13 }}>No data yet</p> : (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={tierData} cx="50%" cy="50%" outerRadius={75} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false} fontSize={11}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
        <div className="card card-p">
          ...
          {websiteData.length === 0 ? <p style={{ color: 'var(--text3)', fontSize: 13 }}>No data yet</p> : (
            <ResponsiveContainer width="100%" height={180}>
              <PieChart>
                <Pie data={websiteData} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" label={({ name, value }) => `${value}`} fontSize={11}>
                  {websiteData.map((e,i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                </Pie>
                <Tooltip contentStyle={{ background: '#111a0e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#f0f4ed' }} />
        ...
        <div className="card card-p">
          ...
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={scoreRanges} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
              <XAxis dataKey="name" tick={{ fill: '#6b7280', fontSize: 11 }} />
              <YAxis tick={{ fill: '#6b7280', fontSize: 11 }} allowDecimals={false} />
              <Tooltip contentStyle={{ background: '#111a0e', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 8, color: '#f0f4ed' }} />
              <Bar dataKey="value" fill="#afc28a" radius={[4,4,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div className="card card-p">
          ...
          {topCategories.length === 0 ? <p style={{ color: 'var(--text3)', fontSize: 13 }}>No data</p> :
            topCategories.map((c, i) => (
              <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                ...
                <div style={{ height: 8, width: `${(c.value / topCategories[0].value) * 100}px`, background: CHART_COLORS[i % CHART_COLORS.length], borderRadius: 4, minWidth: 8 }} />
        ...
        <div className="card card-p">
          ...
          {recentSearches.length === 0 ? <p style={{ color: 'var(--text3)', fontSize: 13 }}>No searches yet</p> :
